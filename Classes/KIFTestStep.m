...
+ (id)mm_stepToTapCellInCollectionViewWithAccessibilityLabel:(NSString *)collectionViewLabel atIndexPath:(NSIndexPath *)indexPath {
    NSString *description = [NSString stringWithFormat:@"Step to tap item %d in collection view with label %@", [indexPath row], collectionViewLabel];
    return [KIFTestStep stepWithDescription:description executionBlock:^(KIFTestStep *step, NSError **error) {
        ...
        if (!cell) {
            ...
            KIFTestCondition([indexPath row] < [collectionView numberOfItemsInSection:[indexPath section]], error, @"Row %d is not found in section %d of '%@' table view", [indexPath row], [indexPath section], collectionViewLabel);
